...
-- drop existing --------
	if (object_id('.dbo.f_clr_zip_create_from_directory') is not null) drop function dbo.f_clr_zip_create_from_directory;
...
-------------------------
	...
	go
	create function f_clr_zip_create_from_directory(@directory_name nvarchar(max), @destination_file_name nvarchar(max), @false_when_error bit) returns bit with execute as owner as external name [mssql.extensions.file.zip].[bi_dev.sql.mssql.extensions.file.zip.Utils].CreateZipFromDirectory;
	go
